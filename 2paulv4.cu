...
void forward(NeuralNetwork* nn, float* batch_data, float* hidden,  float * output){
    ...
    kernel_matmul_A_B_<<<grid2_size,block_size>>>(hidden,nn->weights2, output , BATCH_SIZE, OUTPUT_SIZE, HIDDEN_SIZE);
...
//        dZ1 = dA1 * (Z1 > 0)#dreluoutput=drelu(X1
  //      dW1 = X_flat.T @ dZ1# dW1 = X1.T @dreluoutput
    ...
    kernel_matmul_At_B_<<<grid2,block_size>>>(hidden, grad_output,nn->gradweights2,  HIDDEN_SIZE, OUTPUT_SIZE, BATCH_SIZE );
    CUDA_CHECK(cudaGetLastError());
    ...
    kernel_matmul_A_Bt_<<<grid_hidden,block_size>>>(grad_output,nn->weights2,dA1, BATCH_SIZE, HIDDEN_SIZE, OUTPUT_SIZE );
...
void train(NeuralNetwork * nn, float* data, int* labels){
    ...
    // Host output for simple loss computation
